...
describe('test/testTrelloDemand', () => {
  ...
  describe('loadRawData', () => {
    ...
    const DEMANDINFO = { project: 'some project' };
    ...
    it('stores the data if there is at least one story', () => {
      sandbox.stub(trello, 'loadTrelloDemand').resolves([{ some: 'raw' }, { ugly: 'data' }]);
      return trello.loadRawData(DEMANDINFO, merge(processingInfo, { storageFunction }), '2017-08-09')
      .then(() => {
    ...
    it('stores nothing if there are no stories', () => {
      sandbox.stub(trello, 'loadTrelloDemand').resolves([]);
      return trello.loadRawData(DEMANDINFO, merge(processingInfo, { storageFunction }), '2017-08-09')
      .then(() => {
  ...
  describe('loadTrelloDemand', () => {
    ...
    beforeEach(() => {
      sandbox.stub(Rest, 'get').returns({
        on: sandbox.stub().yields(RAWTRELLOSTORY, { statusCode: HttpStatus.OK }),
      });
    });
